    ...
    class StaticPacker
    {
        ...
        public static int epStuff(MethodDef method)
        {
            for (int i = 38; i < method.Body.Instructions.Count; i++)
            {
                ...
                {
                    // 42	0075	callvirt	instance uint8[]
                    // [mscorlib]System.Reflection.Module::ResolveSignature(int32)
                    if (method.Body.Instructions[i + 1].OpCode == OpCodes.Callvirt &&
                        method.Body.Instructions[i + 1].Operand.ToString().Contains("ResolveSignature"))
                    {
        ...
        public static uint findLocal()
        {
            ...
            var tester =
                Program.module.EntryPoint.Body.Variables
                    .Where(i => i.Type.Namespace == "System.Runtime.InteropServices" && i.Type.TypeName == "GCHandle")
                    .ToArray();
            if (tester.Length != 0)
            {
                Local loc = tester[0];
        ...
        private static uint[] arrayFinder()
        {
            ...
            for (int i = 0; i < entryPoint.Body.Instructions.Count; i++)
            {
                if (entryPoint.Body.Instructions[i].OpCode == OpCodes.Stloc_0)
                {
                    ...
                    if (entryPoint.Body.Instructions[i - 1].OpCode == OpCodes.Call &&
                        entryPoint.Body.Instructions[i - 2].OpCode == OpCodes.Ldtoken)
                    {
